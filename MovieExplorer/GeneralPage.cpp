...
void CGeneralPage::ApplyChanges()
{
	...
	SETPREFBOOL(_T("UseVlc"), (bool)m_chkUseVlc.GetCheck());
...
bool CGeneralPage::OnCreate(CREATESTRUCT *pCS)
{
	...
	if (
		...
		!m_chkUseVlc.Create<RButton>(m_hWnd, BS_AUTOCHECKBOX|WS_TABSTOP) ||
	...
	m_chkUseVlc.SetCheck(GETPREFBOOL(_T("UseVlc")));
...
void CGeneralPage::OnSize(DWORD type, WORD cx, WORD cy)
{
	...
	int y = DUY(4);
	MoveWindow(m_grpInterface, DUX(4), y, cx - DUX(8), DUY(83));
	y += DUY(12);
	...
	y += DUY(14);
	MoveCheckBox(m_chkUseVlc, DUX(14), y);
...
void CGeneralPage::OnPrefChanged()
{
	...
	m_chkUseVlc.SetText(_T(" Resume playback in VLC"));
